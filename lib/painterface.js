    ...
    GetPARouteData = function(data, callback)
    {
      ...
      console.log('found ' + localSinks.length + ' sinks in pa.');
		  ...
		  for(i=0;i<localSinks.length;i++)
			{
					existingSink = PASinks.findOne({name: localSinks[i].name, index: localSinks[i].index});
					if(!existingSink)
					{
      ...
      {
          existingSource = PASources.findOne({name: localSources[i].name, index: localSources[i].index});
          if(!existingSource)
          {
      ...
      {
          existingRoute = PARoutes.findOne({sink: localRoutes[i].sink, source: localRoutes[i].source, index: localRoutes[i].index});
          if(!existingRoute)
          {
			...
			cur.forEach(function(sink) {
				...
				for(i=0;i<localSinks.length;i++)
				{
					if(localSinks[i].name == sink.name && localSinks[i].index == sink.index)
					{
						found = 1;
						break;
					}
      ...
      cur.forEach(function(source) {
        ...
        for(i=0;i<localSources.length;i++)
        {
          if(localSources[i].name == source.name && localSources[i].index == source.index)
          {
            found = 1;
            break;
          }
