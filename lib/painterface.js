...
sourceFilters = new Array("module-remap-source.c","module-null-sink.c")
moduleFilter = "module-loopback"

	if(Meteor.isServer)
	{
		PulseAudio = Npm.require('/opt/node_modules/pulseaudio');
		//PulseAudio = Npm.require('/home/djcinnamon/node_modules_master/pulseaudio');
    ...
    SetPASourceVolume = function(data, callback)
    {
      var context = new PulseAudio()
      console.log('Using the pa api to set volume. source=' + data.name + ' volume=' + data.volume)
      context.on('connection', function(){
        context.set_source_volume_by_name(data.name, data.volume, function(success){
          console.log('return code: ' + success);
          context.end();
          callback(null);
        });
			...
			context.on('connection', function(){
  			...
  			context.sink(function(list){
				...
				context.source(function(list){
					for(i=0;i<list.length;i++)
					{
						if(sourceFilters.indexOf(list[i].driver) > -1)
						{
							var newSource = new Source();
							newSource.index = list[i].index;
							newSource.name = list[i].name;
							newSource.volume = list[i].volume;
							if(list[i].description)
...
//Public API	
		...
		DeactivateRoute = function(routeIndex)
    {
      ...
      Routes.remove({index: routeIndex});
		...
		SetSourceVolume = function(sourceName, volume)
		{
			source = new Source();
			source.name = sourceName;
			source.volume = volume
			SetVolume = Meteor._wrapAsync(SetPASourceVolume);
			SetVolume(source)
			Sources.update({name: sourceName}, {$set: {volume: volume}});
		}	
	}
